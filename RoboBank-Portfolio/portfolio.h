...
struct AccountSetting {
 public:
    ...
    int64_t fee_flat;     ///< Flat fee amount applied to transactions or maintenance.
};
 ...
 *
 * Provides the fundamental operations for querying account data
...
class IAccount {
 public:
     ...
     * @param note Optional transaction note.
     */
    virtual void ApplyInterest(int32_t days, int32_t basis, 
        int64_t ts, std::string note) = 0;
 ...
 *
 * Provides shared functionality for accounts, such as
 ...
 public:
    ...
    */
    ...
    void Fee(int64_t amount, int64_t ts,
        std::string note = nullptr) override;
    void ApplyInterest(int32_t days, int32_t basis, 
 ...
 *
 * Inherits from BaseAccount and represents a checking account type.
 ...
 *
 * The Portfolio class acts as a centralized manager for multiple accounts. 
...
class Portfolio {
 ...
 private:
    using AccountsMap = std::unordered_map<std::string, 
