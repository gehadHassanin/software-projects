...
int main() {
  ...
  std::cout
      << "You can choose any transaction you want to do from this list..\n";
  std::cout << "========================================\n";
  ...
  while (1) {
    ...
    if (transaction == "AddAccount") {
      ...
      std::cout << "number of accounts = " << Portfolio::CountAccounts()
                << std::endl;
      ...
      for (int32_t iter = 0; iter < counts; iter++) {
        ...
        std::cout << "please, enter your id: ";
        std::cin >> id;
        records.push_back(TxRecord((TxKind)type, amount, time_stamp, note, id));
      }
      ...
      Portfolio::ApplyFromLedger(trans_type, amounts, ts, notes, ids, counts);
      ...
      if (Portfolio::Transfer(tr)) {
        std::cout
            << "Do you want to appear new balances of accounts? (yes/no): ";
        std::cin >> out;
        if (out == "yes") {
          std::unique_ptr<IAccount> &from_account = Portfolio::Get(from_id);
          std::unique_ptr<IAccount> &to_account = Portfolio::Get(to_id);
          std::cout << "balance account(1) = " << from_account->Balance()
                    << std::endl;
          std::cout << "balance account(2)= " << to_account->Balance()
                    << std::endl;
        } else {
          // DO NOTHING
        }
      ...
      std::cout << "Total exposure= " << Portfolio::TotalExposure()
                << std::endl;
    } else {
      // DO NOTHING
    }
