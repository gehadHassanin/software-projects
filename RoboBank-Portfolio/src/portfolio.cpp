...
TxRecord::TxRecord(TxKind kind, int64_t amount, int64_t time_stamp,
                   std::string note, std::string id)
    : kind(kind), amount(amount), time_stamp(time_stamp), note(note), id(id) {}

BaseAccount::BaseAccount(std::string id, const AccountSetting &setting,
                         int64_t balance)
    : id_(id), balance_(balance), setting_(setting) {}
...
void BaseAccount::Deposit(int64_t amount, int64_t ts, std::string note) {
  balance_ = ApplyDeposit(balance_, amount);
  Record__(kDeposit, amount, ts, note);
}

void BaseAccount::Withdraw(int64_t amount, int64_t ts, std::string note) {
  balance_ = ApplyWithdrawal(balance_, amount);
  Record__(kWithdrawal, amount, ts, note);
}

void BaseAccount::Fee(int64_t amount, int64_t ts, std::string note) {
  balance_ = ApplyFee(balance_, amount);
  Record__(kFee, amount, ts, note);
}

void BaseAccount::ApplyInterest(int32_t days, int32_t basis, int64_t ts,
                                std::string note) {
  int64_t rate = SimpleInterest(balance_, setting_.apr, days, basis, "HalfUp");
  balance_ += rate;
...
void BaseAccount::Apply(const TxRecord &tx) {
  switch (tx.kind) {
  case kDeposit:
    Deposit(tx.amount, tx.time_stamp, tx.note);
    break;
  case kWithdrawal:
    Withdraw(tx.amount, tx.time_stamp, tx.note);
    break;
  case kFee:
    Fee(tx.amount, tx.time_stamp, tx.note);
    break;
  case kInterest:
...
SavingsAccount::SavingsAccount(std::string id, const AccountSetting &setting,
                               int64_t balance)
    : BaseAccount(id, setting, balance) {}

CheckingAccount::CheckingAccount(std::string id, const AccountSetting &setting,
                                 int64_t balance)
    : BaseAccount(id, setting, balance) {}
...
int32_t Portfolio::CountAccounts() { return accounts_.size(); }
...
void Portfolio::ApplyAll(const std::vector<TxRecord> &records) {
  for (auto &iter : records) {
    if (Is_find__(iter.id)) {
      ...
      account->Apply(iter);
      RecordTransaction__(iter.kind, iter.amount, iter.time_stamp, iter.note,
                          iter.id);
    } else {
      // DO NOTHING
    }
...
void Portfolio::ApplyFromLedger(const TxKind trans_type[], int32_t amount[],
  ...
  for (int32_t iter = 0; iter < count; iter++) {
    TxRecord tx(trans_type[iter], amount[iter], time_stamp[iter], note[iter],
                id[iter]);
...
bool Portfolio::Transfer(TransferRecord tr) {
  ...
  if (it_fromId != accounts_.end() && it_toId != accounts_.end()) {
    ...
    RecordTransaction__(kWithdrawal, tr.amount, tr.time_stamp, tr.note,
                        from_account->Id());
    RecordTransaction__(kDeposit, tr.amount, tr.time_stamp, tr.note,
                        from_account->Id());
    return true;
  } else {
    return false;
...
void Portfolio::RecordTransaction__(TxKind kind, int64_t amount, int64_t ts,
